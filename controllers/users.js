...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    const user = await User.create({ username, password, name, email, logoUrl, coverPhotoUrl, city, coords, logoWidthPercentage, joinDate: new Date() });
...
exports.getFlyerFront = async (req, res, next) => {
  try {
    return res.render('flyer');
  } catch (err) {
    next(err);
  }
};

exports.getFlyerBack = async (req, res, next) => {
  try {
    return res.render('flyer-back');
  } catch (err) {
    next(err);
  }
};
